...
export const createPermission = async (data: { key: string; description: string }): Promise<IPermission> => {
	const existing = await Permission.findOne({ key: data.key });
	if (existing) {
		throw new ApiError("Permission with this key already exists", HTTP_STATUS.CONFLICT);
	}
...
export const updatePermission = async (id: string, data: { key?: string; description?: string }): Promise<IPermission> => {
